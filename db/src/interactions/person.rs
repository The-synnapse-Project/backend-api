...
impl PersonInteractor {
    ...
    pub fn new(conn: &mut DbConnection, person: &models::Person) -> QueryResult<usize> {
        ...
        match &result {
            Ok(rows) => info!("Created person with ID: {}, affected {} rows", person.id, rows),
    ...
    pub fn get_by_email(conn: &mut DbConnection, req_email: &str) -> QueryResult<models::Person> {
        ...
        let result = match conn {
            DbConnection::Sqlite(conn) => person.filter(email.eq(req_email)).first::<models::Person>(conn),
            DbConnection::Pg(conn) => person.filter(email.eq(req_email)).first::<models::Person>(conn),
        };
    ...
    pub fn update(
        ...
        use crate::schema::person::dsl::*;
        info!("Updating person with ID: {} to name: {} and email: {}", 
